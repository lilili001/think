...
class UserModel extends Model {
    ...
    protected $_auto=array(
        //1.新增数据的时候 给count加1 在控制器 add方法执行create的时候 就自动完成给count设置为1了
        //array('count','1'),
        //2.function，给密码加密，加密类型为sha1，sha1函数PHP内置  sha1 或 md5 给user加密 3代表任何情况下 
        //array('user','sha1',3,'function'),
        //把email字段的值填充到user字段冲去
        //array('user', 'email', 3, 'field'),
        //callback，给用户名加前缀
        //array('user', 'updateUser', 3, 'callback', '_'),
        //ignore，用于修改时密码留空时，忽略修改 如果user原来非空，第二次是'',就不修改,还是按原来的
        array('user', '', 2, 'ignore'),
    );
    //回调函数 $str是接传过来的字符串 user $prefix是 '_'
